...
export const SearchPage: React.FC = () => {
  ...
  const searchResults = useMemo(() => {
    ...
    return ALL_PRODUCTS.filter((product) => {
      ...
      const vendor = product.vendor?.name.toLowerCase() || ''
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {searchQuery ? (
          <p className="text-gray-600 mb-8">
            Found <span className="font-bold text-lg">{searchResults.length}</span> result
            {searchResults.length !== 1 ? 's' : ''} for "<span className="font-semibold">{searchQuery}</span>"
          </p>
        ) : (
          <p className="text-gray-600 mb-8">Showing all products</p>
        )}
        ...
        {searchResults.length === 0 ? (
          <div className="bg-white rounded-lg shadow-md p-12 text-center">
            ...
            <Link to="/products" className="inline-block bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition">
              Browse All Products
            </Link>
          </div>
        ) : (
            ...
            {searchResults.map((product) => (
              <Link key={product.id} to={`/product/${product.id}`}>
                <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition transform hover:-translate-y-1">
                  ...
                  {/* Content */}
                  <div className="p-4">
                    <h3 className="font-bold text-lg text-gray-800 line-clamp-2 mb-2">{product.name}</h3>
                    ...
                    {/* Vendor */}
                    <div className="text-xs text-gray-500 mb-3">Sold by: {product.vendor?.name}</div>
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 transition disabled:bg-gray-400 disabled:cursor-not-allowed"
                    >
